...
bit0 = n/a
*/
	const u8 frame_end[4] = {0x04, 0x14, 0x24, 0x34};//不同帧尾
	...
	if (cnt == 25)
	{
		...
		if ((datatmp[0] == 0x0F && (datatmp[24] == 0x00 || datatmp[24] == frame_end[frame_cnt])))//正常数据
		{
			cnt = 0;
...
//		ANO_DT_SendString(Str_RED,"OK!");
	}
	//无信号
	else
	{
		//失控标记置位
		// ANO_DT_SendString(Str_RED,"NO!");
