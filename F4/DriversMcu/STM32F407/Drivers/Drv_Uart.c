...
串口6：雷达
*/
#define U1GetOneByte	Tiva_Receive_Data
#define U2GetOneByte	NoUse
// #define U3GetOneByte	Camera_Receive_Data
#define U4GetOneByte	AnoOF_GetOneByte
#define U5GetOneByte	ANO_DT_LX_Data_Receive_Prepare	
#define U6GetOneByte	Radar_GetOneByte	
...
void DrvUart2SendBuf(unsigned char *DataToSend, u8 data_num)
{
    ...
    for (i = 0; i < data_num; i++)
    {
        TxBuffer[count++] = *(DataToSend + i);/*先把数据存入寄存器，再开启中断发送*/
    }
...
u8 U2RxDataTmp[150];
u8 U2RxInCnt = 0;//转入缓冲区下标
u8 U2RxoutCnt = 0;//转出缓冲区下标
/*	数据转入缓冲区	*/
...
void drvU2DataCheck(void)
{
	...
	{
		U2GetOneByte((char*)U2RxDataTmp[U2RxoutCnt++]);
		if(U2RxoutCnt >= 100)
			U2RxoutCnt = 0;
	}
}
...
{
    u8 com_data;
	/* 处理overload错误，对SR   DR寄存器读操作*/
    if (USART2->SR & USART_SR_ORE) //ORE中断，处理串口溢出
    {
        com_data = USART2->DR;
    }
    ...
    if (USART_GetITStatus(USART2, USART_IT_RXNE))
    {
        ...
        com_data = USART2->DR;
		drvU2GetByte(com_data);//转入寄存器
    }
    //发送（进入移位）中断
    if (USART_GetITStatus(USART2, USART_IT_TXE))
    {
        USART2->DR = TxBuffer[TxCounter++]; //写DR清除中断标志，TxCounter和count作为位置指针
        if (TxCounter == count)
        {
