...
_rt_tar_un rt_tar;			//期望值
_pwm_st pwm_to_esc;			//电调输出
_fc_bat_un fc_bat;			//电池状态
_fc_att_un fc_att;			//姿态角数据
_fc_att_qua_un fc_att_qua;	//姿态四元数数据
_fc_vel_un fc_vel;			//速度数据
...
u8 END_FLAG = 1;
u8 OneKey_flag = 0;
u8 OneKey_mod_change = 99;

void Onekey_lock(void)//一键锁桨
{
...
void RC_Data_Task(float dT_s)
{
	...
	if ( rc_in.fail_safe == 0)
	{
        ...
        else if(rc_in.rc_ch.st_data.ch_[ch_6_aux2] < 1700)//
		{
			if(OneKey_mod_change != 1)
			{
				mod_2_yaw = 1;
				mod_flag = 1;
				OneKey_mod_change = 1;
        ...
        /*AUX4 一键锁桨*/
		if(rc_in.rc_ch.st_data.ch_[ch_8_aux4] < 1200)//直接启动解锁
		{
//			My_onekey_unlock();
		}
        else if(rc_in.rc_ch.st_data.ch_[ch_8_aux4] > 1700)//直接启动锁浆
		{
			My_onekey_lock();
		}
...
//		rt_tar.st_data.vel_z = 0;	 //天向速度，厘米每秒
		//########################################################################
		//=====
		dt.fun[0x41].WTS = 1; //将要发送rt_tar数据。准备发送标志位置1
		//失控保护标记复位
