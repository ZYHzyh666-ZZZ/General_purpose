...
void ANO_DT_SendString(u8 color , const char *str)//发送字符串
{
	...
	data_to_send[_cnt++]=0XFF;//上位机地址
	data_to_send[_cnt++]=0xA0;//功能码ID
	data_to_send[_cnt++]=0;
	data_to_send[_cnt++]=color;
	
	u8 _cnt2=0;
	data_to_char[_cnt2++]=0xAA;
	
	u8 i = 0;
	while(*(str+i) != '\0')
	{
		data_to_send[_cnt++] = *(str+i);
		data_to_char[_cnt2++]= *(str+i++);
		if(_cnt > 50)
			break;
	}
	
	data_to_char[_cnt2++]= 0XCD;
	data_to_char[_cnt2++]= 0XCD;
	...
	ANO_DT_LX_Send_Data(data_to_send, _cnt);	
}
...
void ANO_DT_Send_MY_DATA(u8 number, int len, ...)//发送灵活帧
{
	u8 _cnt = 0;
	u8 check_sum1 = 0, check_sum2 = 0;
	int data = 0;
	...
	data_to_send[_cnt++] = 0xAA;
	data_to_send[_cnt++] = 0XFF;//上位机地址
	data_to_send[_cnt++] = number;//功能码ID
	data_to_send[_cnt++] = 1;
