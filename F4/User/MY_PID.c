...
_PID_parameter  PID_1;    //X
_PID_parameter  PID_2;    //Y
...
void MY_PID_Init(void)
{
  ...
  /*PID参数初始化*/
    _PID_Init(&PID_1, 0.4f, 0.0f, 0.01f, 20.0f, -20.0f);      //X环
    _PID_Init(&PID_2, 0.4f, 0.0f, 0.01f, 20.0f, -20.0f);      //Y环
    _PID_Init(&PID_3, 0.11f, 0.0f, 0.001f, 18.0f, -18.0f);    //锁定
}
...
int PID_calculate_user_high(float exp)//高度PID计算
{
    ...
    if(begin_land == 1)
    {
        ...
        {
            PID_HIGH.limit_max = 28.0f;
            PID_HIGH.limit_min = -28.0f;
        }
...
int PID3_updata(float exp)
{
  ...
  // else
    e_exp = exp;
  return _PID_Traditional_computing(PID_3, e_exp);//传统式PID
}
...
{
    return _PID_Increment_computing(PID_2, exp);  
}
...
{
    return _PID_Traditional_computing(PID_3, exp);  
}
